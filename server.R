...
shinyServer(function(input, output,session) {
  ...
  truncvalue <- reactive({
    if(is.null(input$truncation))
      input$truncation[1] <- 425
    else
      return(as.double(input$truncation[1]))
    })
...
OL <- eventReactive(list(input$MegaDB$datapath,input$truncation), { ####----
 ...
 #   myconn <- odbcDriverConnect('Driver={Microsoft Access Driver (*.mdb, *.accdb)}; DBQ=//GOA/MyDocs/M/Mike.Russell/Analysis/R/Moose_R_Files/359_AUS Distance Survey Data Management Tool_20150223.accdb')
  ...
  #  transflown.MD <- as.data.frame(transflown)
  ...
  #  transflown.MD$DistancePerp <- " "
  #  transflown.MD$MUDE.GroupSize <- " "
  #  transflown.MD$Covariate.1 <- " "
  #  transflown.MD$Covariate.2 <- " "
  #  transflown.MD <- unique(transflown.MD)
   ...
   # datasheet.2 <- unique(datasheet.2)
    ...
    datasheet.2.MD <- unique(datasheet[ which(datasheet$MUDE.GroupSize >0),])
   # datasheet.2.MD <- unique(datasheet.2.MD)
   ...
   # DistanceInput2.MD <- as.data.frame(cbind(object = as.numeric(DistancePreInput.MUDE$ID), Region.Label= DistancePreInput.MUDE$Stratum,Area = as.numeric(DistancePreInput.MUDE$Stratum.Area), Sample.Label = as.numeric(DistancePreInput.MUDE$Transect.ID), Effort = as.numeric(DistancePreInput.MUDE$Transect.Length), distance= as.numeric(DistancePreInput.MUDE$DistancePerp), size=as.numeric(DistancePreInput.MUDE$MUDE.GroupSize),CC=as.factor(DistancePreInput.MUDE$Covariate.1), Activity=as.factor(DistancePreInput.MUDE$Covariate.2)))
    DistanceInput2.MD <- unique(DistanceInput.MD)
    ddf.1.mude <- ds(DistanceInput2.MD, key="hn", adjustment = "cos", truncation = list(left=0, right=425))
...
#    moos.hr_herm <- ds(DistanceInput2, truncation = truncvalue(), key="hr", adjustment = "herm")
    ...
    list(ddf.1.moos = ddf.1.moos, model_result_df = model_result_df, Calf_n = Calf_n, Cow_n = Cow_n, datasheet=datasheet, transflown = transflown, strat_num=strat_num, MOOS_n=MOOS_n,Calf_ratio=Calf_ratio, Bull_ratio=Bull_ratio, Bull_n=Bull_n, WTD_n=WTD_n, MUDE_n=MUDE_n,WAPT_n=WAPT_n, ddf.1.mude=ddf.1.mude, truncvalue = truncvalue(), best.moos=best.moos )
  })

  output$MOOS_DF <- renderPlot({ plot(OL()$ddf.1.moos, main=paste("Global detection function for moose, HN-Cos, truncation=",425))})
  output$MOOS_QQ <- renderPlot({ ddf.gof(OL()$ddf.1.moos$ddf) })
  output$MUDE_QQ <- renderPlot({ ddf.gof(OL()$ddf.1.mude$ddf) })
  output$MOOS_TAB = DT::renderDataTable(OL()$model_result_df, options = list(lengthChange=FALSE), caption=paste("Model results for candidate set of default models for moose. Truncation distance was ",OL()$truncvalue) )
